# source: https://github.com/AmazaspShumik/sklearn-bayes/blob/master/skbayes/linear_models/bayes_logistic.py
...
def logistic_regression_path(X, y, pos_class, mask_val, fit, get_intercept, initial_coef=None, fit_intercept=False):
    mask = (y == pos_class)
    y_bin = np.ones(y.shape, dtype=np.float64)
    y_bin[~mask] = mask_val
    w0 = initial_coef
    coef_, sigma_ = fit(X, y_bin, w0=w0)
    intercept = None
    if fit_intercept:
        intercept, coef_ = get_intercept(coef_)
    return coef_, sigma_, intercept


class BayesianLogisticRegression(LinearClassifierMixin, BaseEstimator, metaclass=ABCMeta):
    '''
    Superclass for two different implementations of Bayesian Logistic Regression
    '''
    def __init__(self, n_iter, tol, fit_intercept, verbose, coef=None, mask_val=0., n_jobs=1):
        self.n_iter        = n_iter
        self.tol           = tol
        ...
        self.n_jobs        = n_jobs
        # Initial values for result variables
        self.coef_ = None
        self.sigma_ = None
    ...
    def fit(self, X, y):
        ...
        #     else:
        #         pos_class = classes_[i]
        #     mask = (y == pos_class)
        #     y_bin = np.ones(y.shape, dtype=np.float64)
        #     y_bin[~mask] = self._mask_val
        #     w0 = self.initial_coef[i, :] if self.initial_coef is not None else None
        #     coef_, sigma_ = self._fit(X, y_bin, w0=w0)
        #     if self.fit_intercept:
        ...
        initial_coef = self.initial_coef if self.initial_coef is not None else [None] * len(classes_)
        result = Parallel(n_jobs=self.n_jobs, verbose=self.verbose,
                          **_joblib_parallel_args(prefer='processes'))(
            path_func(X, y, pos_class=pos_class,
                      fit=self._fit, get_intercept=self._get_intercept,
                      initial_coef=initial_coef,
                      mask_val=self._mask_val,
                      fit_intercept=self.fit_intercept)
            for pos_class, initial_coef in zip(classes_, initial_coef)
        )
        self.coef_, self.sigma_, self.intercept_ = zip(*result)
...
class EBLogisticRegression(BayesianLogisticRegression):
    ...
    def __init__(self, n_iter = 50, tol = 1e-3,solver = 'lbfgs_b',n_iter_solver = 15,
                 tol_solver = 1e-3, fit_intercept=True, alpha = 1e-6, coef=None, n_jobs=1, verbose=False):
        super(EBLogisticRegression,self).__init__(n_iter, tol, fit_intercept, verbose, coef=coef, mask_val=-1., n_jobs=n_jobs)
    ...
    def _fit(self, X, y, w0=None):
        ...
        alpha = self.alpha
        n_samples, n_features = X.shape
        w = np.zeros(n_features) if w0 is None else w0

        for i in range(self.n_iter):
            alpha0 = alpha

            # find mean & covariance of Laplace approximation to posterior
            w, d   = self._posterior(X, y, alpha, w)
            mu_sq  = np.sum(w**2)
...
class VBLogisticRegression(BayesianLogisticRegression):
   ...
   [2] Murphy 2012, Machine Learning A Probabilistic Perspective ( Chapter 21 )
    ...
    def __init__(self,  n_iter=50, tol=1e-3, fit_intercept=True,
                 a=1e-4, b=1e-4, coef=None, n_jobs=1, verbose=False):
        super(VBLogisticRegression, self).__init__(n_iter, tol, fit_intercept, verbose, coef=coef, mask_val=0., n_jobs=n_jobs)
        self.a = a
        self.b = b
    ...
    def _fit(self, X, y, w0=None):
        ...
        n_samples, n_features = X.shape
        XY  = np.dot(X.T, (y-0.5))
        w0 = np.zeros(n_features) if w0 is None else w0
    ...
    def _posterior_dist(self, X, l, a, b, XY, full_covar=False):
        ...
        # is there any specific function in scipy that efficiently inverts
